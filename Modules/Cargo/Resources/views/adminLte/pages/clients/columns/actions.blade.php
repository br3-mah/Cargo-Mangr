...
{{-- @if (auth()->user()->can('view-customers') ||
    $user_role == $admin ||
    $user_role == $branch)
    <a href="{{ fr_route('clients.show', $model->id) }}" class="btn btn-sm btn-secondary btn-action-table"
...
@if (auth()->user()->can('edit-customers') ||
    $user_role == $admin ||
    $user_role == $branch)
    <a href="{{ fr_route('clients.edit', $model->id) }}" class="btn btn-sm btn-secondary btn-action-table"
...
@if (auth()->user()->can('delete-customers') ||
    $user_role == $admin ||
    $user_role == $branch)
    <button type="button" data-action="{{ fr_route('clients.destroy', $model->id) }}" data-callback="reload-table"
